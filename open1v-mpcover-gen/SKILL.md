...
封面标题直接使用文章 YAML frontmatter 中的 `title` 和 `description` 属性，原封不动填入，不删改、不精简。

| 字段 | 来源 | 规则 |
|------|------|------|
| 主标题 | YAML `title` | 原文照搬，不做任何修改 |
| 副标题 | YAML `description` | 原文照搬，不做任何修改 |

**风格特殊元素**（在标题之外额外添加）：

| 风格 | 特殊 |
|------|------|
| 01 大字报 | 1个关键词用 `<span class="hl">` 红色高亮 |
| 02 杂志 | 底部加 `meta`（如 VOL.07 · DESIGN）|
| 03 极简 | 无装饰 |
| 04 像素 | 无装饰 |
...
**通用负面提示词**（所有风格必须追加）：
```
--negative-prompt "text, words, letters, numbers, alphabet, characters, writing, caption, title, headline, subtitle, label, watermark, logo, signature, stamp, border, frame, UI, blurry, low quality, distorted, brand, icon, badge, Qwen, Alibaba, model mark"
```

**去水印提示词**（所有风格的提示词末尾必须追加）：
```
Absolutely no watermark, no logo, no signature, no brand mark, no icon, no badge anywhere in the image. Clean image without any overlay marks.
```
...
```
Loose hand-drawn illustration (2.35:1 ultra-wide banner), soft organic style with visible pencil texture and imperfect lines.
[概念隐喻/场景描述], rendered with sketchy brushstrokes and watercolor bleeds, positioned in the RIGHT 60-75% of frame, sized proportionally (not too large, not too small).
Left 35% remains clean with only subtle paper grain — reserved for text overlay with color mask.
Solid muted [色系名] background filling entire canvas, harmonizing with [配色hex].
Artistic texture across full canvas: rough paper grain, loose watercolor wash, pencil sketch undertones.
The illustration feels like a page from a handmade artist's notebook — warm, personal, slightly imperfect.
Absolutely no text, no writing, no letters anywhere in the image.
```

**关键质感要求**：
- 避免 clean geometric / flat design / crisp lines — 这些会让画面过于工业感
- 使用 loose, organic, hand-drawn, pencil sketch, watercolor bleed, imperfect, sketchy 等词
- 整体气质应像手绘笔记本，而非矢量图标

生图目标：2.35:1 宽幅手绘插画，主体偏右且大小适中，左侧留净给遮罩文字。同色系底色铺满整个画布，纹理统一。
