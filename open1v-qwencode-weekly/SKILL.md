...
description: 为 Qwen Code 创建每周产品更新博客文章（周报）。技能负责数据收集（GitHub releases/PRs）、写作规范、文件生成。通用写作规范（去AI味、术语处理等）由 open1v-qwencode-write skill 管理。触发词：写周报、qwencode 周报、产品更新、版本发布说明、功能迭代记录。
author: joeytoday
...
- **用户视角驱动**：每个功能先说 " 之前怎样 "（痛点），再说 " 现在怎样 "（改进），最后说 " 你能用它做什么 "（场景）
- **白话解释技术**：技术术语第一次出现时用括号给一句白话解释
- **不堆砌功能**：功能描述不是罗列，而是讲清楚 " 解决了什么问题 " 和 " 用起来什么感觉 "
- **诚实表达局限**：功能有边界就说边界，不夸大
- **口语化但不随意**：像跟朋友聊天，但保持信息密度

### 功能排序

按用户感知度排序：

1. **全新功能且解决痛点**（如 /goal、/review、/simplify）
2. **直接可见的交互改进**（如 TUI 界面优化、新按钮）
3. **解决具体痛点的小功能**（不一定最亮眼但用户实际遇到）
4. **有现成截图/视频素材的功能**（便于演示）

**排序技巧**：

- 相邻功能之间要有节奏变化：大功能 → 小功能 → 大功能
- 同类功能不要连续出现超过 2 个
- 如果某功能有特别好的演示素材，可以适当提前

### 演示素材

**每个独立段落功能必须配演示占位符**（视频或截图），在 PR 链接之后：
...
周报段落不固定为 "7 个独立段落 + 表格 "，根据本周功能特点灵活组织。
...
- 某模块本周更新 ≥4 个 PR
- 子功能之间有明确的主题关联（如 "Web Shell 交互改进 ""Channel 基础设施 "）
- 单个子功能不足以撑起独立段落

**体验优化章节判断标准**：

- 功能改动小（1-2 行配置、UI 微调、文案修正）
- 用户感知弱（不影响核心工作流）
- 数量多（≥3 个同类小改动）
...
- [ ] 是否按方向分组（2-4 段），而非逐个功能罗列？
- [ ] 每个方向是否用用户视角描述（" 之前怎样→现在怎样→你能用它做什么 "）？
- [ ] 是否避免了营销腔（" 重磅 "" 颠覆 "" 激动人心 "）？

### 功能描述检查

- [ ] 每个功能是否先说痛点，再说改进，最后说场景？
- [ ] 技术术语是否有白话解释？
- [ ] 是否避免了纯功能罗列？
...
- [ ] 标题是否使用 BlogPostHeader 组件，正文无 `#` 标题？
- [ ] 标题中斜杠命令不加反引号，正文中正常加？
- [ ] PR 链接是否在段落末尾统一列出，从 GitHub API 获取？
- [ ] 正文是否无 `---` 分隔线？
- [ ] MDX 代码块格式：开头包裹在 ```mdx 内，末尾 Callout 不包裹？
- [ ] 是否过滤了内部敏感内容？（CodingPlan/ATA/钉钉协作等，百炼不过滤）
- [ ] 版本功能校验：所有 PR 是否已包含在已发布的稳定版中？

### 周报专属检查

- [ ] **源码验证**：每个功能的描述是否已通过 PR body 验证？
- [ ] **段落组织**：是否根据本周功能特点灵活组织？
- [ ] **模块集中段落放最后**，无 " 你能用它做什么 " 列表，每个子功能含用户场景感知？
- [ ] **功能排序**：全新且解决痛点的功能是否排在前面？
- [ ] **标题价值描述**：每个功能是否带完整的用户价值描述？
- [ ] **贡献者列表**：是否完整，每位标注了具体贡献？
- [ ] **演示素材**：独立段落是否标注了演示占位符？
